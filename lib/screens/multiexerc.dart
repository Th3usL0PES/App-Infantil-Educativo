...
class MultExerciseScreen extends StatefulWidget {
  const MultExerciseScreen({super.key});

  @override
  _MultExerciseScreenState createState() => _MultExerciseScreenState();
}

class _MultExerciseScreenState extends State<MultExerciseScreen> {
  ...
  void _onRiveInit(Artboard artboard) {
    ...
    if (controller != null) {
      ...
      this.controller = controller; // Salva o controller
    }
  }

  void _generateQuestion() {
    final random = Random();
    _firstNumber = random.nextInt(10) + 1; // Números de 1 a 10
    _secondNumber = random.nextInt(10) + 1; // Números de 1 a 10
    _correctAnswer = _firstNumber * _secondNumber;

    _options = List<int>.generate(4, (index) {
      if (index == 0) return _correctAnswer;
      return random.nextInt(100); // Gera outras opções aleatórias
    })..shuffle();
  }
  ...
  void _showEndDialog() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('Você completou todas as perguntas de Multiplicação!'),
        backgroundColor: Colors.white,
        actions: [
          TextButton(
            onPressed: () {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Exercício de Multiplicação'),
      ),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Qual é o resultado da multiplicação desses dois números:',
