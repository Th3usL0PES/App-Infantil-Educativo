...
class DivisaoExerciseScreen extends StatefulWidget {
  const DivisaoExerciseScreen({super.key});

  @override
  _DivisaoExerciseScreenState createState() => _DivisaoExerciseScreenState();
}

class _DivisaoExerciseScreenState extends State<DivisaoExerciseScreen> {
  int _currentQuestionIndex = 0;
  int _dividend = 0;
  int _divisor = 1; // Começar com 1 para evitar divisão por zero
  ...
  void _generateQuestion() {
    final random = Random();
    _dividend = random.nextInt(100) + 1; // Garante que o dividendo não seja zero
    _divisor = random.nextInt(10) + 1; // Garante que o divisor não seja zero
    _correctAnswer = (_dividend / _divisor).floor(); // Resposta correta como inteiro

    _options = List<int>.generate(4, (index) {
      if (index == 0) return _correctAnswer;
      return random.nextInt(20); // Gera respostas aleatórias
    })..shuffle();
  }
  ...
  void _showEndDialog() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('Você completou todas as perguntas de Divisão!'),
        backgroundColor: Colors.white,
        actions: [
          TextButton(
            onPressed: () {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Exercício de Divisão'),
      ),
      body: SingleChildScrollView( // Adiciona o SingleChildScrollView aqui
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Qual é o resultado da divisão desses dois números:',
